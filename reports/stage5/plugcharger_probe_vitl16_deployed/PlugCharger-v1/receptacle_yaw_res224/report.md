# Stage-5 — PlugCharger latent scout: `receptacle_yaw` (negative control) — PlugCharger-v1

**Factor:** receptacle ABSOLUTE world yaw (camera-cancelled by mount=receptacle) → maps to intent factor `constraint_region`.
...
- Encoder: `dinov3_vitl16` spatial_mean @ resolution 224 (224 = the deployed P1 encoder exactly, so a PASS is deployment-honest).
- n=120 reset frames (120 seeds), 2-class label {'neg': 60, 'pos': 60} (majority 0.500).
...
- Probe: G1 protocol (IntentHead F=6, d_slot=32, n_epochs=300, factor_idx=`constraint_region`) + direct StandardScaler+LR.

## Result

| feature | dim | majority | shuffled | direct LR ± std | IntentHead-CV ± std | gate |
|---|---|---|---|---|---|---|
| `global_dino (spatial_mean)` | 1024 | 0.500 | 0.517 | 0.950 ± 0.031 | 0.942 ± 0.042 | PASS |

## Verdict

- IntentHead-CV **0.942** vs majority **0.500**, shuffled **0.308**, gate **0.90**.
- UNEXPECTED PASS — the negative control is separable, so the probe may be exploiting the constant in-frame receptacle/background rather than the charger. Treat every PlugCharger PASS as suspect until this is explained.
