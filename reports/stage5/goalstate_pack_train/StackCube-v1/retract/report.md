...
|---|---|---|---|---|---|---|
| `spatial_mean (all)` | 768 | 0.500 | 0.350 | 0.830 ± 0.039 | 0.767 ± 0.136 | FAIL |
| `last5_mean` | 768 | 0.500 | 0.323 | 0.967 ± 0.015 | 0.867 ± 0.183 | FAIL |
| `first_last` | 768 | 0.500 | 0.293 | 0.937 ± 0.016 | 0.920 ± 0.019 | PASS |
| `final_frame` | 768 | 0.500 | 0.347 | 0.967 ± 0.015 | 0.867 ± 0.183 | FAIL |

## Verdict

- Deployed `spatial_mean (all)` IntentHead-CV **0.767** (gate 0.90); best final-state pooling **`first_last` 0.920**.
- POOLING DILUTION — goal_state IS pixel-groundable, but the DEPLOYED spatial_mean-over-clip pooling dilutes it (0.767); a final-state-aware pooling (`first_last` 0.920) recovers it. goal_state is a FINAL-STATE factor; mean-over-trajectory is the wrong pooling for it. Latent-viable ONLY with a per-factor pooling change (caveat: the deployed encoder uses spatial_mean uniformly across tasks / factors -> a StackCube-specific pooling is a reviewer-visible inconsistency unless framed as principled final-state pooling).
